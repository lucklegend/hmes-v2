 <?php
	...
	* @version 0.3
	*/	
	class EExcelViewCreateEquipmentDataEntryFile extends CGridView
	{
		//Document properties
		...
		public function init()
		{
			...
			{			
				...
				$this->objPHPExcel = PHPExcel_IOFactory::load("./protected/modules/inventory/EquipmentDataEntryForm.xlsx");
		...
		public function run()
		{
			if($this->grid_mode == 'export')
			{
				...
				*/
				...
				//Lab Id/Type
				$objValidation = $this->objPHPExcel->getActiveSheet()->getCell('D6')->getDataValidation();
				...
				$objValidation->setFormula1('DropdownLookup!$A$2:$A$'.($this->labCount+1));	// Make sure to put the list items between " and "  !!!
				$this->objPHPExcel->getActiveSheet()->setCellValue('E6','=VLOOKUP(D6,DropdownLookup!$A$2:$B$'.($this->labCount+1).',2,FALSE)');
				
				//classification Id/Type
				$objValidation = $this->objPHPExcel->getActiveSheet()->getCell('F6')->getDataValidation();
				$objValidation->setType( PHPExcel_Cell_DataValidation::TYPE_LIST );
				$objValidation->setErrorStyle( PHPExcel_Cell_DataValidation::STYLE_INFORMATION );
				$objValidation->setAllowBlank(false);
				$objValidation->setShowInputMessage(true);
				...
				$objValidation->setPromptTitle('Pick from list');
				$objValidation->setPrompt('Please pick a value from the drop-down list.');
				$objValidation->setFormula1('DropdownLookup!$C$2:$C$'.($this->classifyCount+1));	// Make sure to put the list items between " and "  !!!
				$this->objPHPExcel->getActiveSheet()->setCellValue('G6', '=VLOOKUP(F6,DropdownLookup!$C$2:$D$'.($this->classifyCount+1).',2,FALSE)');

				//status Id/Type
				$objValidation = $this->objPHPExcel->getActiveSheet()->getCell('O6')->getDataValidation();
				$objValidation->setType( PHPExcel_Cell_DataValidation::TYPE_LIST );
				$objValidation->setErrorStyle( PHPExcel_Cell_DataValidation::STYLE_INFORMATION );
				$objValidation->setAllowBlank(false);
				$objValidation->setShowInputMessage(true);
				...
				$objValidation->setPromptTitle('Pick from list');
				$objValidation->setPrompt('Please pick a value from the drop-down list.');
				$objValidation->setFormula1('DropdownLookup!$E$2:$E$'.($this->statusCount+1));	// Make sure to put the list items between " and "  !!!
				$this->objPHPExcel->getActiveSheet()->setCellValue('P6', '=VLOOKUP(O6,DropdownLookup!$E$2:$F$'.($this->statusCount+1).',2,FALSE)');

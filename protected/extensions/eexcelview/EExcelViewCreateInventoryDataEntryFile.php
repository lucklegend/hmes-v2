...
********/
	...
	* @version 0.3
	*/	
	class EExcelViewCreateInventoryDataEntryFile extends CGridView
	{
		//Document properties
		...
		public function init()
		{
			...
			{			
				...
				$this->objPHPExcel = PHPExcel_IOFactory::load("./protected/modules/inventory/Initial_Inventory_Data_Entry_Form.xlsx");
		...
		public function run()
		{
			if($this->grid_mode == 'export')
			{
				...
				*/
				...
				//supplies Id/name
				$objValidation = $this->objPHPExcel->getActiveSheet()->getCell('A6')->getDataValidation();
				...
				$objValidation->setFormula1('DropdownLookup!$A$2:$A$'.($this->suppliescount+1));	// Make sure to put the list items between " and "  !!!
				$this->objPHPExcel->getActiveSheet()->setCellValue('B6','=VLOOKUP(A6,DropdownLookup!$A$2:$B$'.($this->suppliescount+1).',2,FALSE)');
				
				//suppliers Id/name
				$objValidation = $this->objPHPExcel->getActiveSheet()->getCell('K6')->getDataValidation();
				$objValidation->setType( PHPExcel_Cell_DataValidation::TYPE_LIST );
				$objValidation->setErrorStyle( PHPExcel_Cell_DataValidation::STYLE_INFORMATION );
				$objValidation->setAllowBlank(false);
				$objValidation->setShowInputMessage(true);
				...
				$objValidation->setPromptTitle('Pick from list');
				$objValidation->setPrompt('Please pick a value from the drop-down list.');
				$objValidation->setFormula1('DropdownLookup!$C$2:$C$'.($this->supplierscount+1));	// Make sure to put the list items between " and "  !!!
				$this->objPHPExcel->getActiveSheet()->setCellValue('L6', '=VLOOKUP(K6,DropdownLookup!$C$2:$D$'.($this->supplierscount+1).',2,FALSE)');

	...
	*/	
	class EExcelViewPrintReceipt extends CGridView
	{
		...
		public function renderRow($row)
		{
			...
			//SET ROW HEIGHT
			self::$activeSheet->getRowDimension('9')->setRowHeight(13);
			...
			//foreach($this->receipt as $res){
				...
				self::$activeSheet->getStyle('D23')->getNumberFormat()->setFormatCode('#,##0.00');
				self::$activeSheet->SetCellValue('D23', $total);

				self::$activeSheet->getRowDimension('24')->setRowHeight(7);
				self::$activeSheet->mergeCells('A25:D26');
				self::$activeSheet->getStyle('A25:D26')->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_TOP);
				self::$activeSheet->getStyle('A25:D26')->getAlignment()->setWrapText(true);
				
				self::$activeSheet->getRowDimension('25')->setRowHeight(22);
				self::$activeSheet->SetCellValue('A25', '                                        '.$this->totalInWords);
				
				if($this->receipt->paymentModeId == 1) {
					self::$activeSheet->SetCellValue('A27', '/');}

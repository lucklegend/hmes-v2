...
/**
	    ...
	    $this->toExcel($labs,
	        ...
	        array(
	        	...
	        	'header' => array(
	        					...
	        					'2' => 'MICROLAB',
	        					'3' => 'METROLAB',
	        					'4' => 'SUMMARY OF DOST-III
	        					 RSTL ACCOMPLISHMENT ('.$year.')',
	        				)
	        ),
	        'Excel5'
	    );
	...
	public function actionMonthlySummary()
    {
        ...
        $criteria->condition = 'analysisYear = :year AND analysisMonth = :month AND t.cancelled = :cancelled';
        //$criteria->order = 't.id ASC';
        $criteria->params = array(':year'=>$year, ':month'=>$month, ':cancelled'=>0);
        $criteria->with = array(
                            'sample' => array(
                                'with'=> array('request' => 
                                    array(
                                        'condition' => 'labId = :labId',
                                        'params' => array(':labId'=>$labId),
        ...
        $dataProvider = new CActiveDataProvider(Monthlyanalyses::model(), array(
            'criteria'=>$criteria,
            'pagination'=>false
        ));
		
		$this->render('monthlySummary',
			array(
				'model'=>$model, 
				'dataProvider' => $dataProvider,
				//'rstlID'=>Yii::app()->user->rstlId,
				'labId'=>$labId,
				'month'=>$month,
				'year'=>$year,
		));
    }
    public function actionTestMonthlySummary(){
    	$model=new MonthlyTest();
		$model->unsetAttributes();
		...
		$criteria->condition = 'analysisYear = :year AND t.cancelled = :cancelled';
        //$criteria->order = 't.id ASC';
        $criteria->params = array(':year'=>$year, ':cancelled'=>0);
        $criteria->with = array(
                            'test'=> array(
	                            'condition' => 'labId = :labId',
	                            'params' => array(':labId'=>$labId),
        ...
        $dataProvider = new CActiveDataProvider(MonthlyTest::model(), array(
            'criteria'=>$criteria,
            'sort'=>array(
            	'defaultOrder'=>'t.method ASC',
            ),
            'pagination'=>false

        ));

    	$this->render('testMonthlySummary',
			array(
				'model'=>$model, 
				'dataProvider' => $dataProvider,
				//'rstlID'=>Yii::app()->user->rstlId,
    ...
    public function actionMonthly(){
    	$this->render('monthly', array(//'lab'=>Lab::model()->findByPk($labId),
									   //'accomp'=>$accomp, 
									   //'summarys'=>$summarys, 
									   //'requests2'=>$requests2, 'sampleCount'=>$sampCount,
									   //'total'=>$total
    ...
    public function onlyNumber($number)
	{
		$var = number_format((int)$number);
		return $var;
		//return number_format($number,0,"",",");
	}

    //convert date to short text months
	public function convertDateToString($months)
	{
		switch($months){
			case '01':
				$months = 'JAN';
	...
	//convert date to no zero (eg. 01,02-09)
	public function convertDateToSingleDigit($months)
	{
		switch($months){
			case '01':
				$months = '1';
	...
	//convert rstl id to rstl region
	public function showRstl($rstlId){
	...
	//get from-date in day-month-year format (use for excel export)
	public function getFromDate()
	{
		$from_date = date('d M Y', strtotime($_GET['from_date']));
		return $from_date;
	}
	//get to-date in day-month-year format (use for excel export)
	public function getToDate()
	{
		$to_date = date('d M Y', strtotime($_GET['to_date']));
		return $to_date;
	}
	//get Lab name (use for excel export)
	public function getLab()
	{
		$labId = $_GET['lab'];
		$lab = Lab::model()->findByPk($labId);
		return $lab->labName;
	}
	//get from-date in year-month-day format (use for subtotal in excel export)
	public function subTotalFromDate()
	{
		$date = date('Y-m-d', strtotime($_GET['from_date']));
		return $date;
	}
	//get to-date in year-month-day format (use for subtotal in excel export)
	public function subTotalToDate()
	{
		$date = date('Y-m-d', strtotime($_GET['to_date']));
		return $date;
	}
}

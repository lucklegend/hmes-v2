...
class OrderofpaymentController extends Controller
{
	...
	public function actionCreateOP($id)	{
		$model=new Orderofpayment;

		$findcustomers= new Customer;
		$model2=$findcustomers->findByPk($id);

		$list=$this->listRequest($id);
		$gridDataProvider = new CArrayDataProvider($list, array('pagination'=>false));
		if(isset($_POST['Orderofpayment']))
		{
			$request = Request::model()->findByPk($id);
			$model->customer_id=$id;
			$model->customerName=$model2->customerName;
			$model->address=$model2->completeAddress;
			$request = Request::model()->findByPk($_POST['requestIds']);
			$model->attributes=$_POST['Orderofpayment'];
			$totalpaymentOP = 0;
			if($model->save()){
				foreach($_POST['requestIds'] as $id){
					...
					$paymentitem->cancelled = 0;
					$paymentitem->save();
					$totalpaymentOP = $totalpaymentOP + $request->getBalance2();
				}
				$model->amount=$totalpaymentOP;
				$model->save();
				$this->redirect(array('view','id'=>$model->id));
			}
			
		}
		$nameofCustomer = new CArrayDataProvider($customerName, array('pagination'=>false));
	...
	public function actionPrintPDF($id)
	{
		...
		$orderpayment = Orderofpayment::model()->findByPk($id);
		$paymentitem = $orderpayment->paymentitems;
		
		//var_dump($paymentitem->amount);

		$pdf = Yii::createComponent('application.extensions.tcpdf.orderpaymentPdf', 'P', 'cm', 'A4', true, 'UTF-8');
		$pdf = new orderpaymentPdf(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);

        spl_autoload_register(array('YiiBase','autoload'));
 
 		$pdf->setOrderpayment($orderpayment);
 		$pdf->setPaymentitem($paymentitem);
 		
        // set document information
        $pdf->SetCreator(PDF_CREATOR);  
 
        $pdf->SetTitle($orderpayment->transactionNum);               
        
        $pdf->SetMargins(25,87.15,25);
        ...
        $pdf->Output($orderpayment->transactionNum, 'I');
        //Yii::app()->end();

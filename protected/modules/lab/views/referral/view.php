...
<?php 
echo CHtml::link('<span class="icon-white icon-print"></span> Print Label', $this->createUrl('referral/printBarcode', array('id'=>$referral['id'])), array('class'=>'btn btn-success', 'target'=>'_blank'));
//echo CHtml::link('<span class="icon-white icon-print"></span> Print Label');
?>
<?php 
    $this->widget('zii.widgets.grid.CGridView', array(
    	'id'=>'analysis-grid',
        ...
        'columns' => array(
			...
			array(
                ...
                'value'=>'($data["package"] == 2) ? $data["packages"]["name"] : ($data["package"] == 1 ? "&nbsp;&nbsp;&nbsp;".$data["testname"]["testName"] : $data["testname"]["testName"])',
				'type'=>'raw',
    		...
    		array(
				...
				'value'=>'($data["methodReference_id"] == 0 ? " - " : $data["methodreference"]["method"])',
				'type'=>'raw',
    			'htmlOptions' => array('style' => 'padding-left: 10px;'),
			),
    		//'quantity',
    		...
    		array(
				...
				'footer'=>
						...
						Yii::app()->format->formatNumber($discount).'<br/><b>'.
						Yii::app()->format->formatNumber($computeTotal).'</b>',
    			'footerHtmlOptions'=>array('style'=>'text-align: right; padding-right: 10px;'),
			),
        ),
    ));
